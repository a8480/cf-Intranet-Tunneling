...
auth_email=""                                       # 登入cloudflare 用户
auth_method="global"                                 # 设为“global” Set to "global" for Global API Key or "token" for Scoped API Token
auth_key=""                                         # 您的API令牌或全局API密钥 Global API Key Your API Token or Global API Key
zone_identifier=""                                  # API 区域识别码 Can be found in the "Overview" tab of your domain
record_name="alst.dedg.cm"                                      # #IP更新同步的DNS(你自己要更新dns的域名) Which record you want to be synced
ttl="300"                                          # 更新时间， 可以不用改 Set the DNS TTL (seconds)
proxy="true"                                       #IP(小黄云)受保护 Set the proxy to true or false
